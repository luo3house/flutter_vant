...
enum VanCalendarType { single, multiple, range }
...
class VanCalendar extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return VanCalendarState();
  }
}

class VanCalendarState extends State<VanCalendar> {
  ...
  VanCalendarType get type => widget.type ?? VanCalendarType.single;
  ...
  @override
  void didUpdateWidget(covariant VanCalendar oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    final selectionRange = () {
      if (type != VanCalendarType.range) return null;
      return Tuple2(
    ...
    onDayTap(DateTime date) {
      ...
      if (type == VanCalendarType.single) {
        setState(() => values = [date]);
        widget.onChange?.call(values);
      } else if (type == VanCalendarType.multiple) {
        ...
        widget.onChange?.call(values);
      } else if (type == VanCalendarType.range) {
        if (values.isEmpty) {
          // select from
          setState(() => values = [date]);
